  ...
  return (
    <PageShell>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
          ...
          <FormField label="목표 기간" htmlFor="targetYears" suffix="년">
            <Input id="targetYears" type="number" variant="ghost" className="text-right tabular-nums" {...register("targetYears", { valueAsNumber: true })} />
          </FormField>
      ...
      <div className="mt-3 rounded-2xl border border-border bg-card p-5 shadow-sm">
        <p className="mb-4 text-xs font-semibold text-muted-foreground">🏷️ 내 조건</p>
        <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
          <FormField label="지역" htmlFor="regionType">
            <NativeSelect id="regionType" variant="ghost" {...register("regionType")}>
              {regionTypes.map((r) => (
          ...
          <FormField label="보유 주택 수" htmlFor="housingCount">
            <NativeSelect id="housingCount" variant="ghost" {...register("housingCount", { valueAsNumber: true })}>
          ...
          <FormField label="가구 특성" htmlFor="tag">
            <NativeSelect id="tag" variant="ghost" {...register("tag")}>
          ...
          <FormField label="생애최초" htmlFor="isFirstTime">
            <label className="flex cursor-pointer items-center gap-2 px-3 py-2.5 text-sm">
              <input id="isFirstTime" type="checkbox" className="h-4 w-4 rounded border-input accent-primary" {...register("isFirstTime")} />
              <span className="text-foreground">생애최초 구입</span>
            </label>
          </FormField>
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        {ltvResult.ltv === 0 ? (
          <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground">{v.targetYears}년 후 예상 자기자금</p>
          </div>
        ) : (
